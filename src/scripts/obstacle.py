...
class Obstacle():
    ...
    def plotCurve(self, Xi, Yi, Thetai, UL, UR, color="green", lw=0.5):
        r = self.wheel_radius
        L = self.wheel_length
        t = 0
        dt = 0.1
    ...
    def plotSpace(self):
        ...
        circle_2_X = [centX + radii * math.cos(i)
                      for i in np.arange(0, 2 * 3.14, 0.01)]
        circle_2_Y = [centY + radii * math.sin(i)
                      for i in np.arange(0, 2 * 3.14, 0.01)]

        square_1_x, square_1_y = [
            0.25, 1.75, 1.75, 0.25, 0.25], [
            4.25, 4.25, 5.75, 5.75, 4.25]
        rectangle_2_x, rectangle_2_y = [
            3.75, 3.75, 6.25, 6.25, 3.75], [
            5.75, 4.25, 4.25, 5.75, 5.75]
        rectangle_3_x, rectangle_3_y = [
            7.25, 7.25, 8.75, 8.75, 7.25], [
            4, 2, 2, 4, 4]
    ...
    def isCircle(self, i, j, center, radius):
        ...
        if ((i - center_x) ** 2 + (j - center_y) **
                2) <= (radius + self.r + self.c) ** 2:
            return True
        else:
            return False
    ...
    def exploredList(self, node, parentNode, actionIndex):
        ...
        self.whichAction.append(actionIndex)
    ...
    def plotPath(self, path, trace_index):
        for i in range(len(path)):
            ...
            self.plotCurve(Xi, Yi, Thetai, UL, UR, color="blue", lw=1.2)
            plt.pause(0.2)
        plt.ioff()
    ...
    def explorationPlot(self):
        for i in range(len(self.plotData_X)):
            ...
            actionIndex = self.whichAction[i]
            UL, UR = self.actions[actionIndex][0], self.actions[actionIndex][1]
